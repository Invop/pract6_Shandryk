# -*- coding: utf-8 -*-
"""python_practice_6.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12s3Dvbd2pYyWbg42oES4dlttKEI0ZzEg
...
*текстова відповідь\
 це рядки документації, що використовуються в Python для пояснення коду. Їх можна додати до модулів, класів, функцій та методів.
\
...
print(f"Сума {a} та {b} дорівнює {sum}")
...
"
Returns the longest case-insensitive common prefix of two strings.

The longest common prefix is the longest string that prefixes both str1 and str2.
This function compares the strings in a case-insensitive manner.
...
Returns:
The longest common prefix of str1 and str2 in lowercase, or an empty string
...
longest_common_prefix("PYTHON", "Pyjamas")
'py'
...
def longest_common_prefix(str1: str, str2: str) -> str:
  ...
  longest_prefix = ""
  for i in range(min(len(lower_str1), len(lower_str2))):
    if lower_str1[i] == lower_str2[i]:
      longest_prefix += lower_str1[i]
    else:
      break
...
print(longest_common_prefix("PYTHON", "Pyjamas"))
#print(longest_common_prefix(123, "world")) #ERR
...
""
  ...
  Args:
      total_loan_amount: The total amount of the loan (in hryvnia).
      real_annual_interest_rate: The real annual interest rate (as a decimal).
      loan_term: The term of the loan (in months).

  Returns:
      The monthly payment amount (in hryvnia).

9. Використовуючи документацію із завдання 8, реалізуйте відповідну функцію.
"""

def calculate_monthly_payment(total_loan_amount, real_annual_interest_rate, loan_term):
  """
  This function calculates the monthly payment for a loan.

  Args:
      total_loan_amount: The total amount of the loan (in any currency).
      real_annual_interest_rate: The real annual interest rate (as a decimal).
      loan_term: The term of the loan (in months).

  Returns:
      The monthly payment amount (in the same currency as the loan amount).
  """
  monthly_interest_rate = real_annual_interest_rate / 12
  number_of_payments = loan_term
  monthly_payment = (total_loan_amount * monthly_interest_rate) / (1 - (1 + monthly_interest_rate) ** (-number_of_payments))

  return monthly_payment

total_loan_amount = 10000
real_annual_interest_rate = 0.05
loan_term = 60

monthly_payment = calculate_monthly_payment(total_loan_amount, real_annual_interest_rate, loan_term)

print(f"The estimated monthly payment is {monthly_payment:.2f} currency units.")
...
def get_value_from_dict(dict, key):
  """
  This function gets a value from a dictionary by key and returns it,
  and in case the key is not found, it catches the KeyError.
...
try:
  ...
  value = get_value_from_dict(my_dict, "non-existent-key")
...
def get_value_from_dict(dict, key):
  """
  This function gets a value from a dictionary by key and returns it,
  and in case the key is not found, it uses an if-else statement.
...
value = get_value_from_dict(my_dict, "non-existent-key")
print(f"Value for key 'non-existent-key': {value}")
...
def get_value_from_dict(dict, key):
  """
  This function gets a value from a dictionary by key and returns it.
  In case the key is not found, it uses the dictionary's 'get' method
  to return a default value (None by default).
  ...
  Returns:
    The value corresponding to the key, or a default value (None by default)
...
def get_age_from_user():
  """
  This function prompts the user for their age, converts it to an integer, and returns it.
...
def get_age_from_user():
  ...
  while True:
    ...
    if age_str.isdigit():
      ...
      print(f"'{age_str}' is not a valid number.")
...
def get_age_from_user():
  """
  This function prompts the user for their age, converts it to an integer, and checks if it is within the range of 18 to 120.
  ...
  Raises:
    ValueError: If the input is not a number, a negative number, or not within the range of 18 to 120.
  """
  ...
  while True:
    try:
      ...
      if age < 0:
        ...
        raise ValueError("Age must be within the range of 18 to 120 years.")
      return age
...
def generate_errors():
  """
  This function generates the maximum number of different types of errors.
  """
  try:
    1 / 0
...
def generate_errors_handled():
  """
  This function demonstrates how to handle potential errors gracefully.
  Fixed
  """
  try:
    result = 1 / 2
  ...
  try:
    large_number = 10 ** 10
    print(f"Large number (may overflow for extremely large powers): {large_number}")
  except OverflowError:
    print("Error: Calculation resulted in overflow.")

  try:
    concatenated = "a" + str(1)
    print(f"Concatenated string: {concatenated}")
  except TypeError:
    print("Error: Cannot concatenate string and integer directly.")
  ...
  try:
    ...
    print(f"Object attribute: {my_object.attribute}")
  except AttributeError:
    print("Error: Object has no such attribute.")
